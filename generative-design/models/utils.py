...
# e_name = ['E11', 'E22', 'E33', 'G23', 'G31', 'G12', 'v21', 'v31', 'v32', 'v12', 'v13', 'v23']
e_name = ['e15', 'e24', 'e31', 'e32', 'e33']
...
def torch_tensor2s(c_tensor):
    s_vec = torch.ones([c_tensor.shape[0], len(e_name)])*1e-6
...
def x2newptb(x_decoded,ptb_mask):
    k = 0
    for i in range(nodesInit.shape[0]):
        for j in range(nodesInit.shape[1]):
            if ptb_mask(i,j) == 1:
                nodesInit[i,j] = nodesInit[i,j] + x_decoded(k)
                k = k+1